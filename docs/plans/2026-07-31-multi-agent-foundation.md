...
- 用右侧面板展示 AgentTree，并展示每个 Agent 的完整工作过程。
...
AgentTree 只包含可交互的 Agent，不包含 summarizer 和 decider。
根 Agent 与 subagent 的区别只由 `parent_agent_id` 表示。

每个 Agent 节点包含 `agent_id`、`name`、`parent_agent_id` 和 `status`。
`agent_id` 是持久化与事件标识，`name` 是当前 conversation 内唯一的显示和路由名称。
`parent_agent_id` 为空的是根 Agent，其余 Agent 通过父节点形成任意深度的树。

每个 Agent 自己判断何时唤醒、创建和管理 summarizer 与 decider。
每次唤醒都是独立的一次性运行，不支持暂停、steer 或接收 agent 消息。
`summarizer-<序号>` 和 `decider-<序号>` 仅标识所属 Agent 内部的运行记录。
每个 Agent 分别计数，并在该 Agent reset context 后从 1 重新计数。
...
memory manager 事件使用所属 Agent 的 `agent_id`，并通过 `memory_manager_run_id` 标识具体运行。
`memory_manager_run_id` 为空时，事件来自 Agent 本身。
memory manager 记录 fork 后收到的任务和执行过程，不复制所属 Agent 的既有时间线。
统一记录替代 `MemoryManagerRunLogger` 后，删除该类及旧 JSONL 写入逻辑。

前端保存 `agent_id -> timeline`，主区域继续显示 worker。
右侧面板按父子关系显示 AgentTree 和状态。
用户选择一个 Agent 后，面板展示其时间线和内部 memory manager 运行记录。

## 实施顺序与暂不实现

先建立 AgentTeam、Agent 内部 memory manager 和独立记录的骨架。
